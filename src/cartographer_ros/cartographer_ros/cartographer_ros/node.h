 ...
 private:
  ...
  cartographer_ros_msgs::StatusResponse TrajectoryStateToStatus(
      ...
      const std::set<
          ...
          valid_states);
  const NodeOptions node_options_; // 节点参数配置，由构造函数传入

  tf2_ros::TransformBroadcaster tf_broadcaster_;    // 发布tf变换
  ...
  ::ros::NodeHandle node_handle_; // 全局
  ...
  std::unordered_set<std::string> subscribed_topics_;           // 所有轨迹订阅的话题名集合
  std::unordered_set<int> trajectories_scheduled_for_finish_;   // 正在结束的轨迹集合
