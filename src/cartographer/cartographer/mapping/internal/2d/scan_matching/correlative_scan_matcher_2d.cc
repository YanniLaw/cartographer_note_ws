...
SearchParameters::SearchParameters(const double linear_search_window,
  ...
  // 根据论文里的公式 求得角度分辨率(角度步长) angular_perturbation_step_size
  // cos(theta) = 1 - (r^2) / (2*d_max^2),其实就是计算激光最远距离时一个栅格对应的角度
...
std::vector<sensor::PointCloud> GenerateRotatedScans(
    const sensor::PointCloud& point_cloud, // 预测的点云
    const SearchParameters& search_parameters) {
  std::vector<sensor::PointCloud> rotated_scans;
  // 生成 num_scans 个旋转后的点云
  rotated_scans.reserve(search_parameters.num_scans);
  // 起始角度(num_scans = 2*num_angular_perturbations + 1)
...
// 将旋转后的点云集合按照预测出的平移量进行平移, 获取(旋转后的点云)平移后的点在地图中的(栅格)索引
std::vector<DiscreteScan2D> DiscretizeScans(
    const MapLimits& map_limits, const std::vector<sensor::PointCloud>& scans,// 不同角度旋转的点云集合
    const Eigen::Translation2f& initial_translation) { // 预估计的初始位置
  ...
  for (const sensor::PointCloud& scan : scans) {
    ...
    discrete_scans.back().reserve(scan.size()); // scan.size() 一帧点云中点的数量
