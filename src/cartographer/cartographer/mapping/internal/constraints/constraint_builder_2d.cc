...
void ConstraintBuilder2D::MaybeAddConstraint(
  ...
  // 在队列中新建一个指向Constraint数据的指针(里面的数据是空的)
  ...
  // 将计算约束这个任务 添加到 finish_node_task_的依赖项中(finish_node_task_要在该约束计算之后完成之后再进行)
  /// 这里是否要先检查其是不是空的???
...
// 约束计算完成之后(返回的结果为需要执行优化)执行一下回调函数
void ConstraintBuilder2D::WhenDone(
  ...
  // 将回调函数赋值给when_done_,callback其实就是pose_graph的HandleWorkQueue函数
 ...
 * @param[in] initial_relative_pose 约束的初值 T_submap_node
...
void ConstraintBuilder2D::ComputeConstraint(
  ...
  // Step:1 得到节点在local frame下的坐标 T_local_node = T_local_submap * T_submap_node
  ...
  float score = 0.;
  transform::Rigid2d pose_estimate = transform::Rigid2d::Identity(); // T_local_node
  ...
  // pose_estimate 是 节点在 loacl frame 下的坐标 T_submap_node = T_local_submap ^ -1 * T_local_node
  ...
  if (options_.log_matches()) {
    ...
    if (match_full_submap) {
      ...
      const transform::Rigid2d difference = // 打印初始值与解算值之间的差距
...
void ConstraintBuilder2D::RunWhenDoneCallback() {
  ...
  std::unique_ptr<std::function<void(const Result&)>> callback;
  {
    ...
    // 将计算完的约束(两次优化过程中计算出来的所有约束关系)进行保存
  ...
  // 执行回调函数 (pose_graph_2d中的HandleWorkQueue函数)
  (*callback)(result);
}
