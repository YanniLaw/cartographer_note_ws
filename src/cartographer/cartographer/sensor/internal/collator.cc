 ...
 * @param[in] expected_sensor_ids 需要排序的所有topic名字的集合
 * @param[in] callback 传入的2个参数的回调函数, 实际是CollatedTrajectoryBuilder::HandleCollatedSensorData()函数
 */
void Collator::AddTrajectory(
  ...
  for (const auto& sensor_id : expected_sensor_ids) {
    ...
    queue_.AddQueue(queue_key,
                    ...
                    // 同样　这里也是注册一个回调函数，来自cartographer/mapping/internal/collated_trajectory_builder.cc的构造函数
                    // 真正触发该回调发生在cartographer/sensor/internal/ordered_multi_queue.cc中的Dispatch()函数
                    [callback, sensor_id](std::unique_ptr<Data> data) {
                      callback(sensor_id, std::move(data));
                    });
    queue_keys_[trajectory_id].push_back(queue_key);// 保存相关queue_key
  }
}
...
void Collator::AddSensorData(const int trajectory_id,
  ...
  QueueKey queue_key{trajectory_id, data->GetSensorId()}; // 多传感器队列中的key
