...
void OptimizationProblem2D::AddTrajectoryNode(const int trajectory_id,
  ...
  trajectory_data_[trajectory_id]; // 轨迹只添加一次
}
...
void OptimizationProblem2D::Solve(
  ...
  for (const auto& submap_id_data : submap_data_) {
    ...
    if (first_submap || frozen) {
      ...
      // trajectory.
      // Step: 如果是第一幅子图, 或者是已经冻结的轨迹中的子图(已经建好的图), 不优化这个子图位姿
  ...
  for (const auto& node_id_data : node_data_) {
    ...
    // 如果是已经冻结的轨迹中的节点(已经建好的图), 不优化这个节点位姿
    if (frozen) {
  ...
  for (const Constraint& constraint : constraints) {
    problem.AddResidualBlock(
        ...
        constraint.tag == Constraint::INTER_SUBMAP // 核函数
            ...
            : nullptr,
        C_submaps.at(constraint.submap_id).data(), // 2个优化变量(C_submaps,C_nodes)
