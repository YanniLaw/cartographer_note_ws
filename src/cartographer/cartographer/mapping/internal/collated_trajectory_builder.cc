 ...
 * @param[in] sensor_collator 传入的整理传感器的类,有2种类型，在cartographer/mapping/map_builder.cc构造函数中被实例化
...
CollatedTrajectoryBuilder::CollatedTrajectoryBuilder(
    ...
    : sensor_collator_(sensor_collator),// 默认为sensor::Collator类型
  ...
  // sensor_collator_　有两种类型的实现方式，默认是sensor::Collator
  // sensor::Collator的初始化　cartographer/cartographer/sensor/internal/collator.cc
  // 这里注册了一个回调函数，第一次不进入HandleCollatedSensorData，只有真正发生了回调才会进入
  // 真正触发回调发生在cartographer/sensor/internal/ordered_multi_queue.cc中的Dispatch()函数
  // sensor::Collator 对应　cartographer/sensor/internal/collator.cc
...
// 将数据传入sensor_collator_(默认是sensor::Collator类型)的AddSensorData进行排序
...
/**
 * @brief 处理 按照时间顺序分发出来的传感器数据，回调函数
 * 
...
void CollatedTrajectoryBuilder::HandleCollatedSensorData(
  ...
  // 找不到就新建一个
  if (it == rate_timers_.end()) { // 这里是对返回值进行的判断处理 注意!!!
  ...
  // 最终将排序好的数据送入 GlobalTrajectoryBuilder中的AddSensorData()函数中进行使用
  // 到cartographer/sensor/internal/dispatchable.h
