...
void MaybeAddPureLocalizationTrimmer(
  ...
  if (trajectory_options.pure_localization()) { // 弃用的老参数
    LOG(WARNING)
...
MapBuilder::MapBuilder(const proto::MapBuilderOptions& options)
  ...
  CHECK(options.use_trajectory_builder_2d() ^
        options.use_trajectory_builder_3d()); // 异或检查

  // 位姿图参数文件位于 cartographer/configuration_files/pose_graph.lua
  // 2d位姿图(后端)的初始化
  if (options.use_trajectory_builder_2d()) {
    pose_graph_ = absl::make_unique<PoseGraph2D>(
        options_.pose_graph_options(),
        absl::make_unique<optimization::OptimizationProblem2D>(
            options_.pose_graph_options().optimization_problem_options()), // optimization_problem变量
        &thread_pool_);
  }
  // 3d位姿图(后端)的初始化
...
int MapBuilder::AddTrajectoryBuilder(
  ...
  // 3d的轨迹
  if (options_.use_trajectory_builder_3d()) { // 该参数位于cartographer_ros/configuration_files的最顶层文件中
    // local_trajectory_builder(前端)的初始化
    std::unique_ptr<LocalTrajectoryBuilder3D> local_trajectory_builder;
    if (trajectory_options.has_trajectory_builder_3d_options()) { //　该参数位于cartographer/configuration_files/trajectory_builder.lua
      local_trajectory_builder = absl::make_unique<LocalTrajectoryBuilder3D>(
          trajectory_options.trajectory_builder_3d_options(),
          SelectRangeSensorIds(expected_sensor_ids)); // 距离传感器
    } 
    ...
    // 真正地创建一个 TrajectoryBuilder ，其中 CollatedTrajectoryBuilder 继承了接口 TrajectoryBuilder
    trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
        trajectory_options, sensor_collator_.get(), trajectory_id,
        expected_sensor_ids,
        // 将3D前端与3D位姿图打包在一起, 传入 CollatedTrajectoryBuilder
  ...
  // 2d的轨迹
    ...
    if (trajectory_options.has_trajectory_builder_2d_options()) {
      ...
      local_trajectory_builder = absl::make_unique<LocalTrajectoryBuilder2D>(
          ...
          SelectRangeSensorIds(expected_sensor_ids)); // 这里只提取了距离传感器(lidar/camera都可)
    }

    DCHECK(dynamic_cast<PoseGraph2D*>(pose_graph_.get())); // 后端位姿图优化
    // CollatedTrajectoryBuilder初始化
    // 传入的参数 sensor_collator_ 为sensor::Collator，是由map_builder的构造函数决定
    // TrajectoryBuilder 连接前端与后端，
    // trajectory_builders_ 类型std::vector<std::unique_ptr<mapping::TrajectoryBuilderInterface>>　多条轨迹对应多个TrajectoryBuilder
    //　真正地创建一个TrajectoryBuilder并推进容器，其中CollatedTrajectoryBuilde继承了接口　TrajectoryBuilder
    trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(// 将2D前端与2D位姿图打包在一起, 传入CollatedTrajectoryBuilder
        trajectory_options, sensor_collator_.get(), trajectory_id,
        expected_sensor_ids, // expected_sensor_ids 这里是所有的传感器id信息 // sensor_collator_整理传感器数据，并传入到TrajectoryBuilder中
        CreateGlobalTrajectoryBuilder2D( // GlobalTrajectoryBuilder类型,其实就是把前后端糅合在一起,再把slam结果传出来
            std::move(local_trajectory_builder), trajectory_id,
            static_cast<PoseGraph2D*>(pose_graph_.get()), // 传入的回调local_slam_result_callback
            local_slam_result_callback, pose_graph_odometry_motion_filter)));
  }
  // local_slam_result_callback 赋值到GlobalTrajectoryBuilder中的local_slam_result_callback_
  ...
  // 上述回调的触发具体在：
  // cartographer/mapping/internal/global_trajectory_builder.cc的AddSensorData函数

  // 是否是纯定位模式, 如果是则只保存最近3个submap(应该是用来做map-to-map的匹配)
  MaybeAddPureLocalizationTrimmer(trajectory_id, trajectory_options,
                                  pose_graph_.get());

  // 如果给了初始位姿，一般可以结合上面这句话使用
  // 如果有则要在cartographer/configuration_files/trajectory_builder.lua中进行设置
