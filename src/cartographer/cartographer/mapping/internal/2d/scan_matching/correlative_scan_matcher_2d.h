...
struct SearchParameters {
  // Linear search window in pixel offsets; bounds are inclusive.xy方向的搜索范围
  struct LinearBounds {
    int min_x;
    int max_x;
    int min_y;
  ...
  int num_angular_perturbations;            // 角度切片个数(角度搜索窗口/角度步长)
  double angular_perturbation_step_size;    // 角度分辨率(步长)
  double resolution;                        // 地图分辨率
  int num_scans;                            // 旋转后的点云集合的个数(2*num_angular_perturbations+1)
...
struct Candidate2D {
  ...
  // Index into the rotated scans vector.最外层的角度
  int scan_index = 0;
