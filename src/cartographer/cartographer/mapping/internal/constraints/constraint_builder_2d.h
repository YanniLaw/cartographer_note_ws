...
class ConstraintBuilder2D {
 ...
 private:
  ...
  // 'callback' set by WhenDone(). 实质上就是HandleWorkQueue函数
  ...
  // 正在计算的约束的队列(其实就是前后两个优化的时刻中间所保存的一些约束)
  std::deque<std::unique_ptr<Constraint>> constraints_ GUARDED_BY(mutex_);

  // Map of dispatched or constructed scan matchers by 'submap_id'.每个子图都有一个scan-matcher
  std::map<SubmapId, SubmapScanMatcher> submap_scan_matchers_
      GUARDED_BY(mutex_);
  std::map<SubmapId, common::FixedRatioSampler> per_submap_sampler_; // 每个子图相对应的采样器
