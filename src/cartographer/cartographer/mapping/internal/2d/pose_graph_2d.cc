...
/**
 * @brief 构造函数
 * 参数文件位于cartographer/configuration_files/pose_graph.lua
...
// 只有在新生成子图的时候才会在 optimization_problem_ 中加入子图的global pose,其他情况下直接返回子图id
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
    const int trajectory_id, const common::Time time,// 节点对应的时间
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) { // 前端生成的活跃子图
  ...
  // 只有slam刚启动建图时子图的个数才为1
  if (insertion_submaps.size() == 1) {
    // If we don't already have an entry for the first submap, add one.
    // 如果判断指定id的submap_data的size为0(也就是第一帧数据到来时), 这条轨迹上还没有添加submap的pose
    if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) { 
      ...
      // 将该submap的global pose(x,y,theta)加入到optimization_problem_中
    ...
    CHECK_EQ(1, submap_data.SizeOfTrajectoryOrZero(trajectory_id));// 确保添加进去了
    ...
    /// CHECK下data_.submap_data时候有其他地方进行更新???
    CHECK(data_.submap_data.at(submap_id).submap == insertion_submaps.front());
    // 因为是第一个submap, 那就把刚刚建立的submap的id返回
    return {submap_id};
  }

  CHECK_EQ(2, insertion_submaps.size()); // 非初始化的时候一般都是两个活跃子图

  // 获取 优化问题中的submap_data 的末尾 trajectory_id
  ...
  // data_.submap_data新加的是insertion_submaps.back()，而optimization_problem_新加的是insertion_submaps.front()
  ...
  if (data_.submap_data.at(last_submap_id).submap ==
      insertion_submaps.front()) { // 前端生成了一个新的子图
    ...
    optimization_problem_->AddSubmap(
        ...
        // 其实就是T_g_first * T_frst_second(相对变换)
        first_submap_pose *
            constraints::ComputeSubmapPose(*insertion_submaps[0]).inverse() *
            constraints::ComputeSubmapPose(*insertion_submaps[1])); // ComputeSubmapPose其实得到的是在local下的pose
 ...
 * 
 * @param[in] constant_data 节点数据的指针(前端产生的节点数据[不会被改变]，包括节点在local系下的位姿，滤波后点云等)
 * @param[in] trajectory_id 轨迹id
 * @param[in] insertion_submaps 活跃的子地图指针的vector(前端生成的active_submaps_)
 * @param[in] optimized_pose 当前节点在global坐标系下的坐标(可以被优化)
 * @return NodeId 返回新生成的节点id
 */
NodeId PoseGraph2D::AppendNode(
    std::shared_ptr<const TrajectoryNode::Data> constant_data, // 位于cartographer/mapping/trajectory_node.h
  ...
  // 向节点列表中添加一个新的节点(并保存该节点的相关数据,包括前端生成的一些const数据，以及该节点在全局坐标系下的位姿)
  const NodeId node_id = data_.trajectory_nodes.Append(
      trajectory_id, TrajectoryNode{constant_data, optimized_pose}); //　TrajectoryNode定义位于cartographer/mapping/trajectory_node.h
  // 节点总个数加1
 ...
 * 
 * @param[in] constant_data 节点信息(前端匹配产生的信息，包括节点在local系下的位姿，滤波后的点云等)
 * @param[in] trajectory_id 轨迹id
 * @param[in] insertion_submaps 活跃的子地图 active_submaps(前端生成的)
...
NodeId PoseGraph2D::AddNode(
  ...
  // 将节点在local坐标系下的坐标转成global坐标系下的坐标(其实是间接通过submap的pose算的)
  const transform::Rigid3d optimized_pose(
      GetLocalToGlobalTransform(trajectory_id) * constant_data->local_pose); // local_pose: tracking in local

  // 向节点列表加入节点,并得到节点的id(添加节点数据[前端传来的数据以及全局位姿]以及子图数据[有新的子图插入时])
...
void PoseGraph2D::AddTrajectoryIfNeeded(const int trajectory_id) {
  // 如果不存在就添加map中
  data_.trajectories_state[trajectory_id]; //　map默认添加方式,如果有该关键字则会添加失败
  ...
  CHECK(data_.trajectories_state.at(trajectory_id).deletion_state ==
        InternalTrajectoryState::DeletionState::NORMAL); // 默认添加后状态即为NORMAL
...
// 将 把数据分发的imu数据加入到优化问题中 这个任务放入到任务队列中
      ...
      for (const auto& observation : landmark_data.landmark_observations) {
        data_.landmark_nodes[observation.id].landmark_observations.emplace_back(
            PoseGraphInterface::LandmarkNode::LandmarkObservation{
                ...
                observation.landmark_to_tracking_transform,//T_tracking_landmark
 ...
 * @param[in] submap_id submap的id(data_.submap_data数据中的id,状态必须为kFinished)
 */
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
  ...
  {
    absl::MutexLock locker(&mutex_);
    // 所有submap必须是 SubmapState::kFinished 的状态才进行回环检测
    ...
    // 获取传入node 和该submap中的所有node 中较新的时间
    ...
    // 如果节点和子图属于同一轨迹(建图时 或者纯定位时定位的节点与子图), 或者时间小于阈值
    ...
    // 才进行 全局搜索窗口 的约束计算(对整体子图进行回环检测)
      ...
      maybe_add_global_constraint = true; // 纯定位时查找回环
    }
  ...
  if (maybe_add_local_constraint) {
    ...
    const transform::Rigid2d initial_relative_pose =  // T_submap_node
 ...
 * @param[in] insertion_submaps active_submaps(前端生成的活跃子图)
 * @param[in] newly_finished_submap 是否是新finished的submap(由前端插入时确定) insertion_submaps.front()
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
  ...
  {
    absl::MutexLock locker(&mutex_);
    // 获取节点信息数据(新的节点数据是在AppendNode函数里面加入)
    const auto& constant_data =
        data_.trajectory_nodes.at(node_id).constant_data;
    
    // 获取 trajectory_id 下的该节点正处于活跃状态下的子图的SubmapId(活跃子图id，最多为2个)
    // 如果子图为初次创建，则会加入到后端优化问题中去(这里的submap_id,在submap_data以及optimization_problem_都是一样的)
    submap_ids = InitializeGlobalSubmapPoses(
        node_id.trajectory_id, constant_data->time, insertion_submaps);
    CHECK_EQ(submap_ids.size(), insertion_submaps.size()); // submap_data的id

    // 获取这两个submap中前一个的id(前一个submap用作匹配)
    const SubmapId matching_id = submap_ids.front();
    // 计算该Node投影到平面后的位姿(x,y,theta) gravity_alignment是机器人的姿态
    const transform::Rigid2d local_pose_2d =
        transform::Project2D(constant_data->local_pose * // 三维转平面
                             transform::Rigid3d::Rotation(
                                 constant_data->gravity_alignment.inverse()));
    // 计算该Node在global坐标系下的二维位姿 T_global_tracking
    // global_pose * constraints::ComputeSubmapPose().inverse() = globla指向local的坐标变换
    // 按这样理解会好很多: T_g_match * (T_match_local * T_local_tracking) = T_g_match * T_match_tracking 
    ...
    // 遍历2个子图, 将节点加入两个子图的节点列表中, 计算子图原点与及节点间的约束(子图内约束)
    for (size_t i = 0; i < insertion_submaps.size(); ++i) {
      ...
      // 将node_id放到子图保存的node_ids的set中(把节点id添加进入子图数据中)
      data_.submap_data.at(submap_id).node_ids.emplace(node_id);
      // 计算 子图原点 指向 node坐标 间的坐标变换(子图内约束)，即node在该子图中的局部位姿
      // T_submap_local * T_local_tracking = T_submap_tracking
    ...
    // them out here.
    // 找到所有已经标记为kFinished状态的submap的id // data_.submap_data为后端的所有submap数据
    // SubmapState为后台线程中子图的当前状态.在此转换为“kFinished”之后, 所有节点都将尝试与此子图匹配, 进行回环检测. 
    // 同样, 所有新节点都与kNoConstraintSearch状态下的子图匹配.
    for (const auto& submap_id_data : data_.submap_data) {
      if (submap_id_data.data.state == SubmapState::kFinished) {
        CHECK_EQ(submap_id_data.data.node_ids.count(node_id), 0); // 已完成子图中不包括当前node
        finished_submap_ids.emplace_back(submap_id_data.id); // finished_submap_ids局部变量
      }
    }

    // 如果是刚刚finished的submap，也就是insertion_submaps.front()
  ...
  // 这里两个约束计算很有意思:
  // 1, 刚插入的节点与所有已完成的子图(不包括刚完成的子图)约束; 
  // 2, 刚完成子图外的所有节点与刚完成子图间的约束
...
// 根据轨迹状态删除轨迹(由DeleteTrajectory函数引发状态变化)
...
void PoseGraph2D::HandleWorkQueue(
    const constraints::ConstraintBuilder2D::Result& result) { // result为上一次优化到此次优化期间的所有约束
  {
    absl::MutexLock locker(&mutex_);
...

  {
    ...
    TrimmingHandle trimming_handle(this);
    // 进行子图的裁剪, 如果没有裁剪器就不裁剪(定位时调用)
    for (auto& trimmer : trimmers_) {
      trimmer->Trim(&trimming_handle); // 调用的是cartographer/mapping/pose_graph_trimmer.cc下的PureLocalizationTrimmer::Trim()
    } // 最终调用到pose_graph的TrimSubmap函数
    // 如果裁剪器处于完成状态, 就把裁剪器删除掉(轨迹设置为完成状态,则裁剪器就变为完成状态)
    trimmers_.erase(
...
void PoseGraph2D::DrainWorkQueue() {
  ...
  while (process_work_queue) {
    std::function<WorkItem::Result()> work_item;
    {
      ...
      if (work_queue_->empty()) {
        work_queue_.reset(); // 重置unique_ptr为空，delete其关联的指针
        return;
      }
      // 取出第一个任务
  ...
  // 运行到这里，说明队列里运行的lambda表达式的返回值是 kRunOptimization, 实际上pop了好几个任务(无需优化的)
  // 如果work_item()都不需要全局优化, 则直到work_queue_为空, 都不会执行优化
  LOG(INFO) << "Remaining work items in queue: " << work_queue_size;
  // We have to optimize again.
  // 退出循环后, 首先等待计算约束中的任务执行完, 再执行HandleWorkQueue,进行优化
  constraint_builder_.WhenDone(
      [this](const constraints::ConstraintBuilder2D::Result& result) { // 注册回调函数
        HandleWorkQueue(result); // 该函数真正执行的地方在ConstraintBuilder2D::RunWhenDoneCallback()中
      });
}
...
void PoseGraph2D::RunOptimization() {
  ...
  // landmark直接参与优化问题
  // 优化的结果有四个:submap,node,landmark,gps坐标原点(均为global_pose)
  ...
  // 获取优化后的结果
  // submap_data的类型是 MapById<SubmapId, optimization::SubmapSpec2D> SubmapSpec2D储存的是global_pose
  const auto& submap_data = optimization_problem_->submap_data();

  // node_data的类型是 MapById<NodeId, NodeSpec2D>
  // node_data是优化后的所有节点的新位姿,优化的是global_pose
  const auto& node_data = optimization_problem_->node_data();
  ...
  for (const int trajectory_id : node_data.trajectory_ids()) {
    ...
    for (const auto& node : node_data.trajectory(trajectory_id)) {
      ...
      // 将优化后的二维节点位姿旋转到机器人的姿态上得到global_pose(3d)
    ...
    // 根据submap_data最后一个被优化的位姿, 计算global坐标系指向local坐标系的坐标变换 T_global_local
  ...
  for (const auto& submap_id_data : data_.submap_data) {
    ...
    submap_poses.Insert(
        ...
        PoseGraph::SubmapPose{submap_data.submap->num_range_data(),
                              submap_data.pose}); // global_pose
  }
  return submap_poses;
}

// 计算 global frame 指向 local frame 的坐标变换
// (这里其实是用最后一个子图在local系与global系下[优化更新过]的不同位姿来进行计算的,T_global_submap * (T_local_submap)^-1)
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
  ...
  if (begin_it == end_it) {
    ...
    // (其实这里是假设最开始local系和global系原点是对齐的，但是随着local误差飘逸回环检测等逐渐不再对齐了,类似map 和odom的关系)
    else {
      return transform::Rigid3d::Identity();
    }
  }

  // 找到了就获取优化后的最后一个子图的id(在RunOptimization函数中优化更新)
...
void PoseGraph2D::TrimmingHandle::TrimSubmap(const SubmapId& submap_id) {
  ...
  std::set<NodeId> nodes_to_remove; // 只出现在要删除的submap中的节点
  ...
  // delete their corresponding constraint submap matchers to save memory.
  {
    ...
    for (const Constraint& constraint : parent_->data_.constraints) { // 在上一步的约束数据基础上调整
      if (nodes_to_remove.count(constraint.node_id) == 0) {
        constraints.push_back(constraint);
      } else {
        // 与 只出现在要删除的submap中的节点 存在约束关系的子图
    ...
    for (const Constraint& constraint : parent_->data_.constraints) {
      if (constraint.tag == Constraint::Tag::INTRA_SUBMAP) { // 子图内约束
        continue;
      } 
