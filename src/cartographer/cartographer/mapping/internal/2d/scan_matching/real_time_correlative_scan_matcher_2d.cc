...
// 计算点云在指定像素坐标(搜索空间)位置下与ProbabilityGrid地图匹配的得分(调用一个该函数计算的是一次搜索位置的得分)
float ComputeCandidateScore(const ProbabilityGrid& probability_grid, // 用于匹配的子图
                            const DiscreteScan2D& discrete_scan, // 该角度下点云所有点经过初始平移过后的像素坐标
                            int x_index_offset, int y_index_offset) {
  float candidate_score = 0.f;
  for (const Eigen::Array2i& xy_index : discrete_scan) {
    // 对每个点都加上像素坐标的offset, 相当于对点云初始位置进行平移
    const Eigen::Array2i proposed_xy_index(xy_index.x() + x_index_offset,
                                           xy_index.y() + y_index_offset);
    // 获取搜索位置占用的概率
...
RealTimeCorrelativeScanMatcher2D::GenerateExhaustiveSearchCandidates(
  ...
  // 计算候选解的个数(即所有搜索的范围[x,y,theta])
  for (int scan_index = 0; scan_index != search_parameters.num_scans;
       ++scan_index) {
    const int num_linear_x_candidates =
        (search_parameters.linear_bounds[scan_index].max_x -
         search_parameters.linear_bounds[scan_index].min_x + 1); // 2*num_linear_perturbations+1
    const int num_linear_y_candidates =
        (search_parameters.linear_bounds[scan_index].max_y -
         search_parameters.linear_bounds[scan_index].min_y + 1);
    num_candidates += num_linear_x_candidates * num_linear_y_candidates; // 不同角度的xy搜索范围叠加
  }
 ...
 * @brief 相关性扫描匹配 - 计算量很大
 * 
 * @param[in] initial_pose_estimate 预测出来的先验位姿 T_local_gravity
 * @param[in] point_cloud 用于匹配的点云(重力对齐坐标系下)　point_cloud.origin = t_local_gravity
...
double RealTimeCorrelativeScanMatcher2D::Match(
  ...
  // Step: 4 生成所有的候选解(即所有搜索的空间[x,y,theta])
  std::vector<Candidate2D> candidates =
  ...
  // Step: 6 获取最优解
  // std::max_element用于查找范围中的最大元素，它接受容器范围[start，end]，并返回一个迭代器，该迭代器指向给定范围中具有最大值的元素。
...
void RealTimeCorrelativeScanMatcher2D::ScoreCandidates(
    const Grid2D& grid, const std::vector<DiscreteScan2D>& discrete_scans,// 不同旋转角度的点云经过初始平移后在地图中的索引
    const SearchParameters& search_parameters,
    std::vector<Candidate2D>* const candidates) const {
  for (Candidate2D& candidate : *candidates) { // 遍历的是所有搜索空间
    ...
    // 对得分进行加权
    candidate.score *=
        std::exp(-common::Pow2(std::hypot(candidate.x, candidate.y) * // std::hypot(x,y) 平方和再开方
