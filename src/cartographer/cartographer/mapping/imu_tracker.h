 ...
 * 并根据IMU的线加速度来确定重力的方向, 并使用重力的方向来对姿态进行校准(其实就是做姿态融合)
 */
class ImuTracker {
 public:
 ...
 private:
  const double imu_gravity_time_constant_;  // g
  common::Time time_;
  common::Time last_linear_acceleration_time_;
  Eigen::Quaterniond orientation_;  // 全局姿态 q_w_k+1
  Eigen::Vector3d gravity_vector_;  // 全局重力方向
