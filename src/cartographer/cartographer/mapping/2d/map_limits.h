 ...
 *             角度正方向以x轴正向为0度, 逆时针为正
 * cartographer地图的像素坐标系: 左上角为原点(地图坐标系的max_), 向右为x正方向, 向下为y正方向
 */
class MapLimits {
 public:
  /**
   ...
   * @param[in] max 左上角的坐标为地图坐标的最大值(物理实际值)
  ...
  // Contains().
  // 计算物理坐标点的像素索引(二维像素坐标)，按行主序存储
  Eigen::Array2i GetCellIndex(const Eigen::Vector2f& point) const {
    // Index values are row major and the top left has Eigen::Array2i::Zero()
    // and contains (centered_max_x, centered_max_y). We need to flip and
    // rotate.像素索引的(0,0)对应物理坐标的max_
  ...
  Eigen::Vector2f GetCellCenter(const Eigen::Array2i cell_index) const {
    return {max_.x() - resolution() * (cell_index[1] + 0.5), // 注意这里的顺序
