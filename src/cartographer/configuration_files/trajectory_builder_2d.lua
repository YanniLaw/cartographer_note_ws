...
TRAJECTORY_BUILDER_2D = {
  ...
  max_range = 30.,
  min_z = -0.8,                   -- 雷达数据的最高与最低的过滤, 保存中间值(将3d数据转为2d)
  max_z = 2.,
  ...
  -- 有2种解决 扫描匹配 问题 的方法
  -- 1.通过 类似闭环检测 的方式找到最优匹配，然后作为 先验，再使用ceres_scan_matcher 进行求解
  -- 计算复杂度高，但是很鲁棒，在odom或者imu不准时依然能达到很好的效果
  ...
  -- 2.将前一时刻的位姿作为先验，使用odom或者imu的数据进行预测，以确定scan的最优位姿
  -- ceres匹配的一些配置参数
  ceres_scan_matcher = {
  ...
  -- 只有当scan的 平移、旋转或者时间 超过阈值时 才会被加入到 submap 中，不超过则舍弃(类似于选关键帧)
  ...
  -- TODO(schwoere,wohe): Remove this constant. This is only kept for ROS.移动时通过imu观测10s以确定重力的平均方向
