...
void Task::AddDependency(std::weak_ptr<Task> dependency) {
  ...
  {
    absl::MutexLock locker(&mutex_);
    CHECK_EQ(state_, NEW); // 如果当前任务被分发了则不能添加依赖
    // 如果指针指针成功获取对象
...
// 添加依赖本Task的Task
// 如b依赖a(要先完成a再完成b)，则a-->b， a.AddDependentTask(b), 根据a的状态，改变b的状态
...
void Task::OnDependenyCompleted() {
  ...
  // 依赖的任务减一
  --uncompleted_dependencies_;
  // 该任务所有的依赖都满足了,并且也处于分发状态
  if (uncompleted_dependencies_ == 0 && state_ == DISPATCHED) {
    state_ = DEPENDENCIES_COMPLETED;
    CHECK(thread_pool_to_notify_);
    thread_pool_to_notify_->NotifyDependenciesCompleted(this); // 通知线程池我准备好啦
  }
}

// 执行本任务, 也就是传入的函数work_item_
// 状态: DEPENDENCIES_COMPLETED -> RUNNING -> COMPLETED
// 比如当前任务为a，并依此更新依赖a的任务dependent_tasks_中所有任务状态，如依赖a的b
void Task::Execute() {
  {
    absl::MutexLock locker(&mutex_);
