 ...
 * @param[in] conversion_tables 地图数据的转换表,来自ActiveSubmaps2D
 */
...
void Submap2D::InsertRangeData(
  ...
  CHECK(grid_);
  CHECK(!insertion_finished()); // 必须是未完成的子图才能进行添加
  // 将雷达数据写到栅格地图中(ProbabilityGrid格式)
...
void Submap2D::Finish() {
  ...
  grid_ = grid_->ComputeCroppedGrid(); // 地图裁剪
  // 将子图标记为完成状态
...
/*
  子图相关的option配置
  submaps = { -- 每个submap的scan总数为num_range_data 的2倍，前一半进行初始化而不进行匹配，后一半进行匹配
    num_range_data = 90,          -- 一个子图里插入雷达数据的个数的一半
    grid_options_2d = {
      grid_type = "PROBABILITY_GRID", -- 存储地图的格式, 还可以是tsdf格式
      resolution = 0.05, -- 栅格尺寸
    },
    range_data_inserter参数见ProbabilityGridRangeDataInserter2D类
*/
// ActiveSubmaps2D构造函数(在LocalTrajectoryBuilder2D构造函数中构造)
...
// 将点云数据写入到submap中,cartographer/mapping/internal/2d/local_trajectory_builder_2d.cc 调用
std::vector<std::shared_ptr<const Submap2D>> ActiveSubmaps2D::InsertRangeData(
    const sensor::RangeData& range_data) {
  // 两种情况会新建子图:
  // 1. 刚开始启动建图时;
  // 2. 第二个子图插入节点的数据等于num_range_data时,就新建个子图,因为这时第一个子图应该已经处于完成状态了
  if (submaps_.empty() ||
      submaps_.back()->num_range_data() == options_.num_range_data()) {
    AddSubmap(range_data.origin.head<2>()); // 传入的origin其实是重力对齐坐标系相对于local系的位置
  }
  // 将一帧雷达数据同时写入两个子图中
  for (auto& submap : submaps_) {
    submap->InsertRangeData(range_data, range_data_inserter_.get()); // num_range_data在此处自增
  }
  // 第一个子图的节点数量等于2倍的num_range_data时,第二个子图节点数量应该等于num_range_data
  // 每一个finished的子图都有2*num_range_data个节点(包括刚建图时生成的第一个子图)
...
ActiveSubmaps2D::CreateRangeDataInserter() { // PROBABILITY_GRID_INSERTER_2D
...
// 以当前雷达原点为地图原点创建地图 t: local frame ---->  gravity aligned frame(重力对齐坐标系相对于local系的位置)
std::unique_ptr<GridInterface> ActiveSubmaps2D::CreateGrid(
    const Eigen::Vector2f& origin) {
  // 地图初始大小,100*100个栅格(对应实际面积5m*5m)
  constexpr int kInitialSubmapSize = 100;
  float resolution = options_.grid_options_2d().resolution(); // param: grid_options_2d.resolution 0.05
    ...
    case proto::GridOptions2D::PROBABILITY_GRID:
      return absl::make_unique<ProbabilityGrid>(
          MapLimits(resolution,
                    // 左上角坐标为坐标系的最大值(cartographer地图坐标系为朝上为x朝左为y), origin位于地图的中间
                    origin.cast<double>() + 0.5 * kInitialSubmapSize *
                                                resolution *
                                                Eigen::Vector2d::Ones(),
                    CellLimits(kInitialSubmapSize, kInitialSubmapSize)),
          &conversion_tables_); // ActiveSubmaps2D::conversion_tables_
    // tsdf地图
...
void ActiveSubmaps2D::AddSubmap(const Eigen::Vector2f& origin) {
  ...
  submaps_.push_back(absl::make_unique<Submap2D>( // 调用Submap2D构造函数
      origin, // origin in local
      std::unique_ptr<Grid2D>( // CreateGrid结果其实是ProbabilityGrid,继承自Grid2D类
          static_cast<Grid2D*>(CreateGrid(origin).release())), // 多态的指针转换,release交出智能指针控制权
      &conversion_tables_));// ActiveSubmaps2D::conversion_tables_
}
