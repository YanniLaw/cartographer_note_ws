...
int MapBuilderBridge::AddTrajectory(
  ...
  const int trajectory_id = map_builder_->AddTrajectoryBuilder(
      ...
      // 这里是注册一个回调函数，第一次不进入lambda表达式，只有真正发生回调时才进入表达式内部语句调用OnLocalSlamResult函数
      // 真正触发回调的地方位于cartographer/mapping/internal/global_trajectory_builder.cc中的AddSensorData函数
      // lambda表达式 local_slam_result_callback_
      [this](const int trajectory_id, // 这些都是传入的参数,当this对象被销毁，那么会导致回调失败
             const ::cartographer::common::Time time,
             const Rigid3d local_pose,
             ::cartographer::sensor::RangeData range_data_in_local,
             const std::unique_ptr<
                 const ::cartographer::mapping::TrajectoryBuilderInterface::
                     InsertionResult>) { // InsertionResult定义在cartographer/mapping/trajectory_builder_interface.h
        // 保存local slam 的结果数据 5个参数实际只用了4个 /*回调函数*/
  ...
  // std::unordered_map<int, std::unique_ptr<SensorBridge>>
...
cartographer_ros_msgs::SubmapList MapBuilderBridge::GetSubmapList() {
  ...
  submap_list.header.frame_id = node_options_.map_frame; // map frame
  for (const auto& submap_id_pose :
    ...
    submap_entry.submap_version = submap_id_pose.data.version; // 插入到子图中雷达数据的个数
    submap_entry.pose = ToGeometryMsgPose(submap_id_pose.data.pose); // gloal pose
...
MapBuilderBridge::GetLocalTrajectoryData() {
  ...
  for (const auto& entry : sensor_bridges_) {
    ...
    // 获取local slam 数据,在global_trajectory_builder中的AddSensorData函数进行更新
...
visualization_msgs::MarkerArray MapBuilderBridge::GetConstraintList() {
  ...
  // 将约束信息填充到6种marker里
  for (const auto& constraint : constraints) { // 遍历所有约束
    ...
    if (constraint.tag ==
      ...
      // 相同轨迹内,子图外部约束, 对应第三种与第四种marker(建图模式)
      if (constraint.node_id.trajectory_id ==
          constraint.submap_id.trajectory_id) {
      ...
      // 不同轨迹间的constraint,对应第五种与第六种marker(定位模式)
      else {
    ...
    // 根据子图坐标与约束的坐标变换算出约束的另一头的坐标 T_global_node = T_global_submap * T_submap_node
 ...
 * 回调函数，将传感器数据加入到trajectory_state_data_容器中，按照轨迹索引进行储存
