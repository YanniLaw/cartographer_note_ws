...
// ValueToProbability转换表　extern变量定义:　定义时为该变量分配内存空间，并赋初值(这是个全局变量，在编译时就完成初始化)
...
std::vector<uint16> ComputeLookupTableToApplyCorrespondenceCostOdds(
    float odds) {
  std::vector<uint16> result; // 所以result的值的范围是加上kUpdateMarker的值,即[32769,65536]
  ...
  for (int cell = 1; cell != kValueCount; ++cell) {
    result.push_back(
        CorrespondenceCostToValue(
            ProbabilityToCorrespondenceCost(ProbabilityFromOdds( // 当前栅格概率为0.1-0.9时,对应观测到hit或者miss的概率
