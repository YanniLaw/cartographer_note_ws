...
std::unique_ptr<std::vector<float>> PrecomputeValueToBoundedFloat(
  ...
  // num_values = 65536
  size_t num_values = std::numeric_limits<uint16>::max() + 1; // uint16_t表示 [0,2^16-1] 即[0,65535]
  // 申请空间
  result->reserve(num_values);

  /**
   * 1 byte = 8 bit, uint16 = uint16_t = unsigned short int 占2个字节,16位
   * kUpdateMarker = 2^16 = 32768 那么其补码为: 1000 0000 0000 0000
   * ~kUpdateMarker 每个位都取反,则为:0111 1111 1111 1111
   * static_cast<uint16>(value) & ~kUpdateMarker,位的与运算, 则表示只取右边15位,即范围为0~2^16-1
   */
