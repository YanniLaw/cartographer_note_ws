 ...
 * 
 * @tparam MessageType 模板参数,ros消息的数据类型(消息类型)
 * @param[in] handler 函数指针, 接受传入的函数的地址(传感器数据消息回调)
 * @param[in] trajectory_id 轨迹id
 * @param[in] topic 订阅的传感器topic名字
...
::ros::Subscriber SubscribeWithHandler(
  ...
  return node_handle->subscribe<MessageType>(
      ...
      boost::function<void(const typename MessageType::ConstPtr&)>(
          // c++11: lambda表达式
          // 注册传感器消息的回调函数，在有传感器数据时触发该回调函数
          [node, handler, trajectory_id, topic](const typename MessageType::ConstPtr& msg) {
            (node->*handler)(trajectory_id, topic, msg); // 即下面的HandleXXXXXXXMessage等回调函数
          }));
}
 ...
 * @param[in] tf_buffer tf缓存
 * @param[in] collect_metrics 是否启用metrics,默认不启用
 */
  // * const 常指针，指针的地址是一个常量，也就是指向的对象不能变
  // const * 
Node::Node(
    const NodeOptions& node_options,
    std::unique_ptr<cartographer::mapping::MapBuilderInterface> map_builder, // 基类指针
...
void Node::PublishSubmapList(const ::ros::WallTimerEvent& unused_timer_event) {
  absl::MutexLock lock(&mutex_);
  submap_list_publisher_.publish(map_builder_bridge_.GetSubmapList());// global pose
}
...
/**
 * @brief 新生成一个传感器数据采样器
 * 参数位于　cartographer_ros/cartographer_ros/configuration_files/lx_rs16_2d_outdoor.lua
 * @param[in] trajectory_id 轨迹id
 * @param[in] options 参数配置
 */
...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent& timer_event) {
  absl::MutexLock lock(&mutex_);
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) { // 多条轨迹
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      ...
      // 如果设置为true, 就发布一个在tracking_frame处的pose(global_pose)
      if (node_options_.publish_tracked_pose) {
 ...
 * 传感器的参数文件位于cartographer_ros/configurationfiles中
...
Node::ComputeExpectedSensorIds(const TrajectoryOptions& options) const {
  ...
  */
  ...
  for (const std::string& topic :
       ComputeRepeatedTopicNames(kLaserScanTopic, options.num_laser_scans)) { // num_laser_scans 单线激光雷达的个数
...
int Node::AddTrajectory(const TrajectoryOptions& options) {
  ...
  // SensorId{传感器类型,topic话题名称}
  const std::set<cartographer::mapping::TrajectoryBuilderInterface::SensorId>
      expected_sensor_ids = ComputeExpectedSensorIds(options);

  // 调用map_builder_bridge的AddTrajectory, 添加一条轨迹相关的各类bridge桥和各类处理函数
  /*总结其工作：
  ...
  // 新生成一个传感器数据采样器,规定了传感器采样频率
  AddSensorSamplers(trajectory_id, options);

  // 订阅话题与注册回调函数,是传感器信息的入口
  LaunchSubscribers(options, trajectory_id); // 重点关注这个
  ...
  for (const std::string& topic :
    ...
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<sensor_msgs::LaserScan>(
             ...
             this), // 最终回调的调用函数形式为HandleLaserScanMessage(trajectory_id, topic, msg)
         topic}); // 自定义结构　Subscriber{::ros::Subscriber,topic}
  }
 ...
 * 第2个是传入SensorBridge,进入数据流水线，其最终也会进入前端的PoseExtrapolator以及后端
 * 
 * @param[in] trajectory_id 轨迹id
...
void Node::HandleOdometryMessage(const int trajectory_id,
  ...
  if (odometry_data_ptr != nullptr) {
    extrapolators_.at(trajectory_id).AddOdometryData(*odometry_data_ptr); // 转化为tracking_frame的数据
  }
  sensor_bridge_ptr->HandleOdometryMessage(sensor_id, msg);
}

// 调用SensorBridge的传感器处理函数进行数据处理，最终数据只进入后端
 ...
 * 第1个是传入node的PoseExtrapolator,用于位姿预测与重力方向的确定
 * 第2个是传入SensorBridge,进入数据处理流水线，其最终也会进入前端的PoseExtrapolator以及后端
 * 
 * @param[in] trajectory_id 轨迹id
...
void Node::HandleImuMessage(const int trajectory_id,
  ...
  auto sensor_bridge_ptr = map_builder_bridge_.sensor_bridge(trajectory_id);
  auto imu_data_ptr = sensor_bridge_ptr->ToImuData(msg); // 转到tracking_frame的imu数据
  // extrapolators_使用里程计数据进行位姿预测
  // 添加数据到pose_extrapolator的imu队列中
  if (imu_data_ptr != nullptr) { // cartographer/mapping/pose_extrapolator.cc
...
void Node::HandleLaserScanMessage(const int trajectory_id,
  ...
  // 先获取对应轨迹id的SensorBridge的指针,再调用HandleLaserScanMessage
  map_builder_bridge_.sensor_bridge(trajectory_id)
