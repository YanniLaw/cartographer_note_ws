...
class LocalTrajectoryBuilder2D {
 public:
  ...
  struct MatchingResult {
    common::Time time;                      // scan-match发生的时间
    transform::Rigid3d local_pose;          // tracking frame在局部地图local坐标系下的位姿(scan-match优化后)
    sensor::RangeData range_data_in_local;  // 经过扫描匹配之后位姿校准之后的雷达数据(in local frame)
    // 'nullptr' if dropped by the motion filter.
    std::unique_ptr<const InsertionResult> insertion_result;    // 子图插入结果，包括前端位姿点云数据以及两个活跃子图等
  };
 ...
 private:
  ...
  const proto::LocalTrajectoryBuilderOptions2D options_; // 前端的配置参数
  ActiveSubmaps2D active_submaps_; // 活动中的submap
  ...
  RangeDataCollator range_data_collator_; // 用于对不同的距离传感器进行消息同步，点云排序
};
