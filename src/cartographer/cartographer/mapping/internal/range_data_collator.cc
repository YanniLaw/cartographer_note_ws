...
/**
 * 简单说明一下时间同步的过程: 假设有两中传感器数据过来，分别是a,b 采集到的时间如下所示
 * 由于传过来的时间是激光雷达点云最后一个时间，所以时间同步的开始时间以及结束时间为
 *    |    |       |       |        |      |   打竖线的地方即为处理的区间段!!!
 * a: ------  ------  ------   ------ ------
 * b:    ------  ------  ------  ------  ------
 * 处理完成之后返回的数据把两个传感器的数据都融合到了一起，但是每个点云数据都有明显的标志表示自己来自哪个传感器
 * 另，处理完之后会对原传感器数据进行裁剪，同步过的数据会丢掉
 */

/**
 * @brief 多个距离传感器数据的时间同步
 * 
...
sensor::TimedPointCloudOriginData RangeDataCollator::AddRangeData(
  ...
  // 找到所有传感器数据中最早的时间戳(点云最后一个点的绝对时间)
  for (const auto& pair : id_to_pending_data_) {
    oldest_timestamp = std::min(oldest_timestamp, pair.second.time);
  }
  // current_end_是本次时间同步的结束时间，也是下次同步的开始时间
...
sensor::TimedPointCloudOriginData RangeDataCollator::CropAndMerge() {
  ...
  for (auto it = id_to_pending_data_.begin();
    ...
    const sensor::TimedPointCloud& ranges = it->second.ranges;  // 相对time + xyz(tracking frame)
    const std::vector<float>& intensities = it->second.intensities; // 填充为默认值

    // 以下两句话确定时间同步的处理区间，即选择时间在[current_start_,current_end_]的点
    // 1.找到点云中 最后一个时间戳小于current_start_的点的索引
    auto overlap_begin = ranges.begin();
    while (overlap_begin < ranges.end() &&
           data.time + common::FromSeconds((*overlap_begin).time) < // 加起来就是点云中每个点的绝对时间
               current_start_) {
      ++overlap_begin;
    }

    // 2.找到点云中 最后一个时间戳小于等于current_end_的点的索引
    ...
    // 丢弃点云中时间比起始时(current_start_)早的点, 每执行一下CropAndMerge()打印一次log
    ...
    if (overlap_begin < overlap_end) {
      // 获取下个点云的index, 即当前vector的个数(如果有多个距离传感器的话)
      std::size_t origin_index = result.origins.size();
      result.origins.push_back(data.origin);  // 插入原点坐标(tracking frame到sensor frame的坐标系变换)

      // 获取此传感器时间与集合时间戳的误差, 这个值是大于等于0的
    ...
    // 如果点云每个点都用了, 则可将这个数据进行删除(一般只有某一种传感器时)
    ...
    // 用了一部分的点
      ...
      // 将用了的点删除, 这里的赋值是拷贝(保留同步后的点)data是引用!!!
