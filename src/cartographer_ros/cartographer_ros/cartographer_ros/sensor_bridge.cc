 ...
 * @param[in] trajectory_builder 轨迹构建器 CollatedTrajectoryBuilder类型
 */
...
std::unique_ptr<carto::sensor::OdometryData> SensorBridge::ToOdometryData(
  ...
  // T_odom_baselink = T_odom_basefootprint * (T_baselink_basefootprint)^-1
...
// sensor_msgs::NavSatFix　数据格式 参考https://blog.csdn.net/tiancailx/article/details/109462226
...
void SensorBridge::HandleNavSatFixMessage(
  ...
  const carto::common::Time time = FromRos(msg->header.stamp);
  // 如果不是固定解,就加入一个固定的空位姿
  if (msg->status.status == sensor_msgs::NavSatStatus::STATUS_NO_FIX) { // STATUS_NO_FIX: 不能定位
    trajectory_builder_->AddSensorData(
        sensor_id,
  ...
  // gps与slam坐标系转换基本原理:由于gps结果都是经纬度等信息(球坐标)，所以先将这些数据转换到ecef坐标系(直角坐标)下,
  // 然后以第一帧gps数据得到的局部东北天(或其他水平面坐标)坐标作为一个杆臂值(或者外参)，之后的gps信息与之前的gps信息计算坐标变换(ecef坐标系)
  // 最后把坐标变换值改到局部东北天(或其他水平面坐标)坐标下即可拿到一个真实的尺度信息，但是这个值也是有噪声的，所以还需要跟其他传感器(通常是imu)进行融合
  
  // 确定ecef原点到局部坐标系的坐标变换T_local_ecef
  // ecef(地心地固坐标系，地球质心为原点，z 轴与地轴平行指向北极点，x 轴指向本初子午线与赤道的交点，y 轴垂直于xOz平面(即东经90度与赤道的交点))
  ...
  // 其实就是gps数据相对第一帧gps的相对位姿(局部东北天坐标系下)
  trajectory_builder_->AddSensorData(
...
void SensorBridge::HandleImuMessage(const std::string& sensor_id,
                                    const sensor_msgs::Imu::ConstPtr& msg) {
  std::unique_ptr<carto::sensor::ImuData> imu_data = ToImuData(msg); // 转化到tracking_frame(一般就是imu_link自己)了
  if (imu_data != nullptr) {
    trajectory_builder_->AddSensorData( // cartographer/mapping/internal/collated_trajectory_builder.h
        sensor_id,
...
void SensorBridge::HandleLaserScanMessage(
  ...
  carto::common::Time time; // 以点云中最后一个点的时间为点云时间戳
...
void SensorBridge::HandleLaserScan(
    const std::string& sensor_id, const carto::common::Time time, // 点云最后一个点的时间戳为点云时间戳
    const std::string& frame_id, // 传感器frame
    const carto::sensor::PointCloudWithIntensities& points) { // points　cartographer格式的点云
  ...
  // 意为一帧雷达数据被分成几次处理, 一般将这个参数设置为1(方便畸变矫正处理)
  for (int i = 0; i != num_subdivisions_per_laser_scan_; ++i) {
    ...
    // 生成分段的点云, TimedPointCloud = std::vector<TimedRangefinderPoint>
    carto::sensor::TimedPointCloud subdivision(
        points.points.begin() + start_index, points.points.begin() + end_index);
    if (start_index == end_index) {
      continue;
    }
    // PointCloudWithIntensities里面每个点存的是相对点云最后一个点的相对时间，如果是最后一个点，则为0
    // 所以time_to_subdivision_end表示的是当前分段点云最后一个点在整个完整(未分段)点云中的相对时间
    const double time_to_subdivision_end = subdivision.back().time;
    // `subdivision_time` is the end of the measurement so sensor::Collator will
    // send all other sensor data first.
    // 如果进行了分段处理，则subdivision_time表示当前分出的点云的最后一个点的(绝对)时间
    const carto::common::Time subdivision_time =
        time + carto::common::FromSeconds(time_to_subdivision_end);
    
    auto it = sensor_to_previous_subdivision_time_.find(sensor_id);
    if (it != sensor_to_previous_subdivision_time_.end() &&
        // 当前段点云的时间应该大于等于上一段点云的时间
        it->second >= subdivision_time) {// 去掉可能存在的时间交错的点，确保顺序处理
    ...
    // 更新当前段点云对应sensor_id的时间戳
    sensor_to_previous_subdivision_time_[sensor_id] = subdivision_time;
    
    // 分段之后应该重新设置分段点云的时间,之前是分段点云每个点相对于完整点云最后一个点相对时间
    // 现在是每个点相对于分段点云最后一个的相对时间了
    for (auto& point : subdivision) {
      point.time -= time_to_subdivision_end;
    }
    CHECK_EQ(subdivision.back().time, 0.f); // 其实就是分段之后相当于一段完整的新的点云去处理了
 ...
 * @param[in] time 点云的时间戳(最后一个点的绝对时间)
...
void SensorBridge::HandleRangefinder(
  ...
  if (!ranges.empty()) {
    CHECK_LE(ranges.back().time, 0.f); // 相对时间
  }
  // 利用tf查询激光雷达frame到tracking frame的坐标变换 (cartographer::transform::Rigid3d 格式)
  const auto sensor_to_tracking =
      tf_bridge_.LookupToTracking(time, CheckNoLeadingSlash(frame_id));

  // 以 sensor_frame 到 tracking 的坐标变换为TimedPointCloudData 的 origin
  ...
  if (sensor_to_tracking != nullptr) {
    trajectory_builder_->AddSensorData( // ***cartographer/mapping/internal/collated_trajectory_builder.h
        sensor_id, carto::sensor::TimedPointCloudData{
                       ...
                       // 将点云从雷达坐标系下转到tracking_frame坐标系系下 p_tracking = T_tracking_lidar * p_lidar
