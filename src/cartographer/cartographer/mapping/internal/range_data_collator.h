...
class RangeDataCollator {
 ...
 private:
  ...
  const std::set<std::string> expected_sensor_ids_; // 多个rangedata传感器的id
  // Store at most one message for each sensor.
  std::map<std::string, sensor::TimedPointCloudData> id_to_pending_data_; // 待处理的数据
  common::Time current_start_ = common::Time::min();  // 本次时间同步的开始时间
  common::Time current_end_ = common::Time::min();    // 本次时间同步的结束时间
