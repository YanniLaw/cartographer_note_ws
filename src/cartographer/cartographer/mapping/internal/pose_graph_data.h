...
// 同样, 所有新节点都与kNoConstraintSearch状态下的子图匹配.　ComputeConstraintsForNode函数中更改状态
...
struct InternalTrajectoryState {
  ...
  // 轨迹的状态: ACTIVE, FINISHED, FROZEN, DELETED
...
struct InternalSubmapData {
  std::shared_ptr<const Submap> submap; // 与前端相同的submap数据，包含子图原点在local下的坐标等
  SubmapState state = SubmapState::kNoConstraintSearch; // 子图状态(在前端生成的子图完成后更改为完成状态)
  ...
  std::set<NodeId> node_ids; // 子图所拥有的节点数据(类似于局部地图的关键帧)
};
...
struct PoseGraphData {
  ...
  // submap 在 global 坐标系下的坐标(会被优化) SubmapSpec2D = transform::Rigid2d
  ...
  // Data that are currently being shown.TrajectoryNode定义位于cartographer/mapping/trajectory_node.h
  // 所有的轨迹节点的id与节点相关信息，包括 节点的在global坐标系下的坐标(可优化), 在local系下的坐标与时间(const值)
