...
PrecomputationGrid2D::PrecomputationGrid2D(
    ...
    : offset_(-width + 1, -width + 1),
      wide_limits_(limits.num_x_cells + width - 1,  // 该分辨率下的格子数目
                   limits.num_y_cells + width - 1), // 相比原grid而言，该分辨率下格子总数多(width-1)*(width-1)
  ...
  const int stride = wide_limits_.num_x_cells; // 该分辨率下的x轴格子数目
  ...
  for (int y = 0; y != limits.num_y_cells; ++y) {
    ...
    // grid的索引 x + width 的坐标范围是 [1, width-1], x 的范围是 [-width+1,0)
    for (int x = -width + 1; x != 0; ++x) {
      intermediate[x + width - 1 + y * stride] = current_values.GetMaximum();
      if (x + width < limits.num_x_cells) { // width相当于给了个偏移
...
PrecomputationGridStack2D::PrecomputationGridStack2D(
  ...
  // 保存地图值
  std::vector<float> reusable_intermediate_grid; // (可重用中间网格)
  const CellLimits limits = grid.limits().cell_limits(); //　当前子图x,y方向栅格数
 ...
 * 
 * @param[in] initial_pose_estimate 先验位姿(x,y,theta)
 * @param[in] point_cloud 原点位于local坐标系原点处的点云 /// 应该是位于重力对齐坐标系下
...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
    ...
    const transform::Rigid2d& initial_pose_estimate, // T_local_node
    const sensor::PointCloud& point_cloud, float min_score, float* score,
    transform::Rigid2d* pose_estimate) const {
  CHECK(score != nullptr);
  ...
  // Step: 生成按照不同角度旋转后的点云集合(最高分辨率地图下)
  const std::vector<sensor::PointCloud> rotated_scans =
      GenerateRotatedScans(rotated_point_cloud, search_parameters);

  // Step: 将旋转后的点云集合按照预测出的平移量进行平移, 获取平移后的点在地图中的索引
  // 这里的离散激光点是在最细(高)的分辨率的地图上面
  const std::vector<DiscreteScan2D> discrete_scans = DiscretizeScans(
      limits_, rotated_scans,
      Eigen::Translation2f(initial_pose_estimate.translation().x(),
                           initial_pose_estimate.translation().y()));
  
  // 缩小搜索窗口的大小, 计算每一帧点云在保证最后一个点能在地图范围内时的最大移动范围(最高分辨率地图下)
...
FastCorrelativeScanMatcher2D::ComputeLowestResolutionCandidates(
    const std::vector<DiscreteScan2D>& discrete_scans, // 当前帧点云在最高分辨率下的栅格索引
  ...
  ScoreCandidates(
      precomputation_grid_stack_->Get(precomputation_grid_stack_->max_depth()), // 最低分辨率地图
      discrete_scans, search_parameters, &lowest_resolution_candidates);
...
FastCorrelativeScanMatcher2D::GenerateLowestResolutionCandidates(
    const SearchParameters& search_parameters) const {
  // 相当于最高分辨率栅格地图的多少个格子合成最低分辨率栅格地图的一个格子
  const int linear_step_size = 1 << precomputation_grid_stack_->max_depth(); // 2^6 = 64　
  int num_candidates = 0;
  // 遍历旋转后的每个点云
  for (int scan_index = 0; scan_index != search_parameters.num_scans;
       ++scan_index) {
    // 这些都是通过在最高分辨率地图上进行相应的计算　来获得在最低分辨率地图上的所有候选解
    // X方向候选解的个数
  ...
  for (int scan_index = 0; scan_index != search_parameters.num_scans;
    ...
    for (int x_index_offset = search_parameters.linear_bounds[scan_index].min_x;
      ...
      for (int y_index_offset =
        ...
        // 生成候选解, 存的是候选解与原始点云原点坐标间的偏移量(这个偏移值是在最高分辨率地图下的)
...
// 对所有的候选解进行评分并进行降序排序(包含了旋转角度点云)
void FastCorrelativeScanMatcher2D::ScoreCandidates(
    const PrecomputationGrid2D& precomputation_grid,
    const std::vector<DiscreteScan2D>& discrete_scans, // 旋转不同角度后的点云在(最高分辨率)地图中的栅格索引
    const SearchParameters& search_parameters,
    std::vector<Candidate2D>* const candidates) const { // 该层分辨率地图下的所有候选解(按得分大小进行排序)
  // 遍历所有的候选解, 对每个候选解进行打分
  for (Candidate2D& candidate : *candidates) {
    int sum = 0;
    // xy_index 为这帧旋转后的点云上的每个点对应在地图上的栅格坐标
    for (const Eigen::Array2i& xy_index :
         discrete_scans[candidate.scan_index]) { // discrete_scans[candidate.scan_index]某个角度下的
...
Candidate2D FastCorrelativeScanMatcher2D::BranchAndBound(
  ...
  for (const Candidate2D& candidate : candidates) {
    ...
    // 搜索步长减为上层的一半
    const int half_width = 1 << (candidate_depth - 1); // 2^(candidate_depth - 1)
