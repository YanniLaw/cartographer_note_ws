 ...
 * 回调函数来自cartographer/mapping/internal/collated_trajectory_builder.cc的HandleCollatedSensorData函数
 * @param[in] queue_key 轨迹id与topic名字
 * @param[in] callback void(std::unique_ptr<Data> data) 型的函数
 * 到这里的callback已经是对应sensor_id的callback了
 */
...
void OrderedMultiQueue::Add(const QueueKey& queue_key,
  ...
  auto it = queues_.find(queue_key);
  // 如果queue_key不在queues_中, 就忽略data(这是因为在初始化的时候给每个传感器都分配了key的)
  if (it == queues_.end()) {
 ...
 * 回调位于cartographer/mapping/internal/collated_trajectory_builder.cc的构造函数
 * 3种退出情况:
...
void OrderedMultiQueue::Dispatch() {
  while (true) {
    /*
      时间的选择逻辑其实很简单，轨迹初始化时，选取多传感器中最新时间作为开始时间(因为这样所有的传感器都有值了)，
      其余就选取多传感器中最老时间(数据一个一个的派发)，比如下面这样的数据那么就按照数字顺序依次分发出去
      queues_: 
        (0, scan): {      4,     }
        (0, imu):  {1,  3,   5,  }
        (0, odom): {  2,       6,}
    */
    const Data* next_data = nullptr;  // 带next表示马上要处理的数据
    ...
    for (auto it = queues_.begin(); it != queues_.end();) {
      ...
      if (data == nullptr) {
        ...
        CannotMakeProgress(it->first); // 发布警告信息
        return; // 保证所有的传感器数据都添加到队列中(保证都有传感器数据)
      }

      // 第一次进行到这里(此时为next_data == nullptr)或者data的时间比next_data的时间小(老数据)
      // 就更新next_data, 并保存当前话题的数据队列以及queue_key
      // 总之就是找所有queues_(多种传感器数据)里面时间最老的传感器数据
    ...
    // 所有传感器数据都有值进来才会进入下面的语句
    // 这个时候next_data 就是所有传感器数据中时间最老的了
    ...
    // Step: 2 获取对应轨迹id的所有数据队列中的最小共同时间戳(其实就是都有数据时用最大的那个时间做为基准，相当于做时间同步), 作为轨迹开始的时间
    // 只在轨迹初始化时有效，后续不会再更新这个值了 common_start_time 时，所有的topic 都有了数据了
