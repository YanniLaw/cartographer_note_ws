...
class OccupiedSpaceCostFunction2D {
 public:
  ...
  template <typename T>
  bool operator()(const T* const pose, T* residual) const { // pose维度为三维,residual维度为点云中激光点的数量(该参数放最后)
...
ceres::CostFunction* CreateOccupiedSpaceCostFunction2D(
    ...
    const Grid2D& grid) {
  // 自动求导模板参数: 误差类型，输出维度(误差项)，输入维度(优化变量个数)
  /* template <typename CostFunctor,
        int kNumResiduals,  // Number of residuals, or ceres::DYNAMIC(运行时确定).
        int N0,       // Number of parameters in block 0. 当前参数块的维度，可以有多个参数块
  ...
  */
  return new ceres::AutoDiffCostFunction<OccupiedSpaceCostFunction2D,
      ...
      point_cloud.size()); // 这个才是运行时真正设置的残差维度(会替代上面模板中的残差维度),即点云中激光点的数目
}
