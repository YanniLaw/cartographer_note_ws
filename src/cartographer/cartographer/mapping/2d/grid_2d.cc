...
Grid2D::Grid2D(const MapLimits& limits, float min_correspondence_cost,
    ...
    : limits_(limits),
      correspondence_cost_cells_(
          limits_.cell_limits().num_x_cells * limits_.cell_limits().num_y_cells, // 保存栅格free值的地方
          kUnknownCorrespondenceValue),  // 地图栅格值　0:unknown
...
void Grid2D::GrowLimits(const Eigen::Vector2f& point) {
  GrowLimits(point, {mutable_correspondence_cost_cells()},
             {kUnknownCorrespondenceValue}); // 用kUnknownCorrespondenceValue值来初始化容器
}

// 根据坐标决定是否对地图进行扩大()
void Grid2D::GrowLimits(const Eigen::Vector2f& point, //传入的物理坐标
                        const std::vector<std::vector<uint16>*>& grids, // 可以对多个地图进行操作(莫非是不同分辨率做准备??)
                        const std::vector<uint16>& grids_unknown_cell_values) { // 需保证与grids同样大小
  CHECK(update_indices_.empty());
  // 判断该点是否在地图坐标系内(通过转换成像素坐标进行判断的)
  while (!limits_.Contains(limits_.GetCellIndex(point))) {
    ...
    const MapLimits new_limits(
        limits_.resolution(),
        limits_.max() +
            limits_.resolution() * Eigen::Vector2d(y_offset, x_offset),// 新的max_,origin保持一致
        CellLimits(2 * limits_.cell_limits().num_x_cells,
                   2 * limits_.cell_limits().num_y_cells)); // 新图面积是原来的4倍
    const int stride = new_limits.cell_limits().num_x_cells; // 2倍的旧图num_x_cells
    // 老坐标系的原点在新坐标系下的一维像素坐标(origin中心保持不变)
    const int offset = x_offset + stride * y_offset;
    const int new_size = new_limits.cell_limits().num_x_cells *
                         new_limits.cell_limits().num_y_cells;

    // grids.size()为1 (可以对多个地图同时操作)
