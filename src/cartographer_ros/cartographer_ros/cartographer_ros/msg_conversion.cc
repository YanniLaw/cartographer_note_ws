...
// For sensor_msgs::MultiEchoLaserScan. 多回波激光echoes为std::vector<float> 数据类型
...
LaserScanToPointCloudWithIntensities(const LaserMessageType& msg) {
  ...
  CHECK_GE(msg.range_max, msg.range_min); // 检查距离信息与角度信息
  ...
  for (size_t i = 0; i < msg.ranges.size(); ++i) {
    ...
    if (HasEcho(echoes)) {
      // 如果是laserScan 则返回range信息，如果是多回波激光则返回第一个回波range信息
      const float first_echo = GetFirstEcho(echoes);
      // 满足范围才进行使用
      if (msg.range_min <= first_echo && first_echo <= msg.range_max) {
        const Eigen::AngleAxisf rotation(angle, Eigen::Vector3f::UnitZ());
        const cartographer::sensor::TimedRangefinderPoint point{
            rotation * (first_echo * Eigen::Vector3f::UnitX()), // position(直角坐标)
            i * msg.time_increment};                            // time(相对时间)
        // 保存点云坐标与时间信息
        ...
        if (msg.intensities.size() > 0) {
          ...
          point_cloud.intensities.push_back(GetFirstEcho(echo_intensities)); // 多回波激光则取第一个
        } else {
  ...
  //　ros以第一个点的时间为一帧点云时间戳，cartographer以最后一个点的时间为一帧点云时间戳
  ::cartographer::common::Time timestamp = FromRos(msg.header.stamp);
  if (!point_cloud.points.empty()) {
    const double duration = point_cloud.points.back().time;
    // 以点云最后一个点的时间为点云的时间戳
    timestamp += cartographer::common::FromSeconds(duration); // 转换后的当前帧点云时间戳(绝对时间)
 ...
 * @return cartographer::transform::Rigid3d 计算第一帧GPS数据指向ECEF坐标系下原点的坐标系变换
 */
