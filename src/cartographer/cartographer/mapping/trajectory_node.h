...
struct TrajectoryNode {
  ...
  struct Data {
    ...
    // 'gravity_alignment' frame.自适应体素滤波后的点云(位于重力对齐坐标系下)
    ...
    // The node pose in the local SLAM frame.节点(tracking frame)在local系下的3d位姿
    transform::Rigid3d local_pose;
  };
  ...
  // The node pose in the global SLAM frame.节点在global系下的位姿
